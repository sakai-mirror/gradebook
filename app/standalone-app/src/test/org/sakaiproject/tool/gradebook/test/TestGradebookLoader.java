...
public class TestGradebookLoader extends GradebookLoaderBase {
	...
	public void testLoadGradebooks() throws Exception {
        ...
        for(int i = 0; i < 7; i++) {
            ...
            gradebookManager.createAssignment(gb.getId(), ASN_BASE_NAME + i, new Double(pts), date, Boolean.FALSE,Boolean.FALSE, false);
        }

        // Add an assignment without a due date.
        gradebookManager.createAssignment(gb.getId(), ASN_NO_DUE_DATE_NAME, new Double(50), null, Boolean.FALSE,Boolean.FALSE, false);
        ...
        gradebookManager.createAssignment(gb.getId(), ASN_NOT_COUNTED_NAME, new Double(100), new Date(), Boolean.TRUE,Boolean.FALSE, false);
