	...
	public void transferGradebookDefinitionXml(String fromGradebookUid, String toGradebookUid, String fromGradebookXml) {
		...
		if(category.size() !=0) {
				...
				for (org.sakaiproject.service.gradebook.shared.Assignment obj : gradebookDefinition.getAssignments()) {
					...
					if (assignmentDef.getCategoryName() != null) {
						if(!newCategory) {}
							...
							catId = createCategory(gradebook.getId(), assignmentDef.getCategoryName(), assignmentDef.getWeight(), 0, 0, 0, 0.0, null);
			...
			while(itUpdate.hasNext()){
				...
				createCategory(gradebook.getId(), catObj.getName(), catObj.getWeight(), catObj.getDrop_lowest(), catObj.getDropHighest(), catObj.getKeepHighest(), catObj.getItemValue(), catObj.getIsExtraCredit());				
			}
		}
