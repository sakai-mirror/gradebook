		...
		getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException {
				...
				if (gradeRecord == null) {
					...
					gradeRecord = new AssignmentGradeRecord(assignment, studentUid, score.toString());
				} else {
					gradeRecord.setPointsEarned(score.toString());
				}
	...
	public void transferGradebookDefinitionXml(String fromGradebookUid, String toGradebookUid, String fromGradebookXml) {
		...
		if(category.size() !=0) {
				...
				for (org.sakaiproject.service.gradebook.shared.Assignment obj : gradebookDefinition.getAssignments()) {
					...
					if (assignmentDef.getCategoryName() != null) {
						if(!newCategory) {}
							...
							catId = createCategory(gradebook.getId(), assignmentDef.getCategoryName(), assignmentDef.getWeight(), 0, 0, 0, 0, 0.0, null);
			...
			while(itUpdate.hasNext()){
				...
				createCategory(gradebook.getId(), catObj.getName(), catObj.getWeight(), 0, catObj.getDropLowest(), catObj.getDropHighest(), catObj.getKeepHighest(), catObj.getItemValue(), catObj.getIsExtraCredit());				
			}
		}
...
//	}
	...
	public List getPointsEarnedCourseGradeRecords(final CourseGrade courseGrade, final Collection studentUids) 
	{
		HibernateCallback hc = new HibernateCallback() 
		{
			public Object doInHibernate(Session session) throws HibernateException 
			{
				...
				{
					if(log.isInfoEnabled()) log.info("Returning no grade records for an empty collection of student UIDs in GradebookServiceHibernateImpl.getPointsEarnedCourseGradeRecords(CourseGrade, Collection).");
				...
				{
					if(log.isInfoEnabled()) log.error("Wrong grade type in GradebookServiceHibernateImpl.getPointsEarnedCourseGradeRecords(CourseGrade, Collection)");
					return new ArrayList();
				}
	...
	private double getTotalPointsInternal(final Long gradebookId, Session session, final Gradebook gradebook, final List categories, final String studentId)
	{
		...
		if( gbGradeType != GradebookService.GRADE_TYPE_POINTS && gbGradeType != GradebookService.GRADE_TYPE_PERCENTAGE)
		{
			if(log.isInfoEnabled()) log.error("Wrong grade type in GradebookServiceHibernateImpl.getTotalPointsInternal");
			return -1;
		}
	...
	private	List getTotalPointsEarnedInternal(final Long gradebookId, final String studentId, final Session session, final Gradebook gradebook, final List categories) 
	{
		...
		if( gbGradeType != GradebookService.GRADE_TYPE_POINTS && gbGradeType != GradebookService.GRADE_TYPE_PERCENTAGE)
		{
			if(log.isInfoEnabled()) log.error("Wrong grade type in GradebookServiceHibernateImpl.getTotalPointsEarnedInternal");
			return new ArrayList();
		}
...
//						  } else {
							  gradeDef.setGrade(agr.getPointsEarned());
//						  }
...
//			  Double convertedGrade = convertInputGradeToPoints(gradebook.getGrade_type(), mapping, assignment.getPointsPossible(), gradeDef.getGrade());
			  String convertedGrade = gradeDef.getGrade();
...
//  }
	...
	private Double fixingPointsEarned(Double pointsEarned, Assignment assignment, Gradebook gradebook)
	{
  	...
  	if(pointPossible != null && pointPossible.doubleValue() > 0)
  	{
  		...
  		letterGrade = lgpm.getGrade(calculateEquivalentPercent(new Double(pointPossible), new Double(pointsEarned)));
  ...
  throws GradebookNotFoundException, AssessmentNotFoundException
  {
  	...
  	String assignmentScore = (String)getHibernateTemplate().execute(new HibernateCallback() {
  		public Object doInHibernate(Session session) throws HibernateException {
  			Assignment assignment = getAssignmentWithoutStats(gradebookUid, assignmentName, session);
  			if (assignment == null) {
  				throw new AssessmentNotFoundException("There is no assignment named " + assignmentName + " in gradebook " + gradebookUid);
